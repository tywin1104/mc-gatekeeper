...
import { Button, Form, FormGroup, Label, Input, UncontrolledAlert, Jumbotron,  Spinner, CardBody, Card, Badge} from 'reactstrap';
...
class Application extends React.Component {
  ...
  getUsernameVerificationBadge = () => {
    if(this.state.verified) {
      return <Badge color="success" pill>{i18next.t('Splash.Verified')}</Badge>
    }else {
      return <Badge color="danger" pill>{i18next.t('Splash.NotVerified')}</Badge>
    }
  }
...
render() {
    ...
    if(this.state.errorMsg !== "") {
        ...
        messageBlock = (
            ...
            <span className="alert-inner--text">
                <strong>{i18next.t('Splash.Success')}</strong>{i18next.t('Splash.SuccessMsg')}
            </span>
            </UncontrolledAlert>
        )
    }
    ...
    <Form role="form" onSubmit={this.onSubmit}>
      <FormGroup>
        ...
        <Input type="email" name="email"  required placeholder="example@gmail.com"  value={this.state.email}  onChange={this.handleInputChange} />
      </FormGroup>
      <FormGroup>
        <Label >{i18next.t('Splash.Username')} {this.getUsernameVerificationBadge()}</Label>
        <Input type="text" name="username" disabled={this.state.verified || this.state.loading} required  placeholder="username"  value={this.state.username}  onChange={this.handleInputChange}/>
      </FormGroup>
        <Card>
          <CardBody>
              ...
              <ol>
                <li>{i18next.t('Splash.VerifyStep1')}</li>
                    ...
                    <QRCode style={{display: "none"}}
                      ...
                      includeMargin={false}
                    />
                    <Button  outline size="sm" color="info" onClick={this.downloadQR}> {i18next.t('Splash.DownloadButton')} </Button>
                  </div>
                <li>
                  {i18next.t('Splash.VerifyStep2-1')} <a href="https://my.minecraft.net/en-us/profile/skin" target="_blank">https://my.minecraft.net/en-us/profile/skin</a>, {i18next.t('Splash.VerifyStep2-2')}
                </li>
                <li>{i18next.t('Splash.VerifyStep3')}</li>
                <li>{i18next.t('Splash.VerifyStep4')} <br></br>
                  <Button size="sm" outline style={{display: this.state.loading ? 'none' : '' }} disabled={this.state.verified} onClick={this.onVerifyUsername} color="primary">{i18next.t('Splash.VerifyButton')}</Button>
                  <Spinner style={{display: this.state.loading ? '' : 'none' }} color="primary" />
                </li>
      ...
      <Button disabled={!this.state.verified} color="primary" type="submit"  size="lg">
